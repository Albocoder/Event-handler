...
public class LoginHandler extends HttpServlet {
	...
	private int loginCheck(String user, String pass) {
		ResultSet rs;
		SQLConn connection = new SQLConn("jdbc:mysql://sql2.freemysqlhosting.net:3306/sql287738","sql287738","qE5*eR5%");
		rs = connection.getData();
		try{
			while(rs.next()){
				String username = rs.getString("username");
				String password = rs.getString("password");
				String role = rs.getString("role");
				if(user.equalsIgnoreCase(username) && pass.equals(password) && (role.equalsIgnoreCase("headadmin") || role.equalsIgnoreCase("admin"))){
					return 1;
				}
				else if(user.equalsIgnoreCase(username) && pass.equals(password) && role.equalsIgnoreCase("user")){
					return 2;
				}
			}
			return 0;
